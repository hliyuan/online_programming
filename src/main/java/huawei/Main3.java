 ...
 * 题目描述
 ...
 则对他们所代表的16进制的数进行BIT倒序的操作，并转换为相应的大写字符。如字符为‘4’，为0100b，则翻转后为0010b，也就是2。转换后的字符为‘2’；
    ...
    public static void main(String[] args) {
        ...
        while (in.hasNext()){
            ...
            for (int i = 0; i < s.length(); i++) {
                if(i%2==0) sb1.append(s.charAt(i));
            ...
            if (c1.length>c2.length)    ss.append(c1[c1.length-1]);
            else if (c1.length<c2.length)  ss.append(c2[c2.length-1]);
